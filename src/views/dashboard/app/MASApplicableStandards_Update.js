...
const UserProfile = () => {
    ...
    const deleteproduct = async (id) => {
        let result = await fetch(`http://localhost:5005/MASapplicationStandard/${id} `, {
          method: "Delete",});
      ...
      const updateproduct = async ( ) => {
        console.warn(params);
    let result = await fetch(`http://localhost:5005/MASapplicationStandard/${params.id}`);
    result = await result.json();
    setname( result.name );
      };
    ...
    const handleSubmit = async () => {
        const result = fetch(`http://localhost:5005/MASapplicationStandard/${params.id}`, {
            method: "Put",
